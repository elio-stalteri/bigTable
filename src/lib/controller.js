...
bigTableData.subscribe((value) => {
    ...
    if (value.length > 0) {
        bigTableHeaderConfig.update((prev) =>
            prev && prev.length > 0
                ? prev
                : Object.keys(value[0]).map((h) => ({ name: h, key: h, width: 400 })),
        );
...
filterSearchSortConfig.subscribe((value) => {
    if (Object.keys(value.sort).length > 0) {
        ...
        setTimeout(() => {
            const sortParams = fastSort
                .sort(Object.values(value.sort))
                .by([{ asc: (u) => u.order }]);
...
export const sort = (key) => {
    filterSearchSortConfig.update((prev) => {
        ...
        let lastSorted =
            Object.values(prevSort).length > 0
                ? Math.max(...Object.values(prevSort).map((v) => v.order))
                : 0;
        if (!(key in prevSort)) {
